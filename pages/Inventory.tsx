...
const SkeletonCard: React.FC<{ view: "grid" | "list" }> = ({ view }) => (
  ...
  >
    <div
      className={`${view === "list" ? "md:w-[40%] h-64 md:h-full" : "aspect-[16/10]"} bg-white/5 relative overflow-hidden`}
    >
...
const Inventory: React.FC = () => {
  ...
  const filteredCars = useMemo(() => {
    return cars.filter((car) => {
      ...
      const matchesBody =
        filters.bodyType === "All" || carBodyType === filters.bodyType;
      const matchesFuel =
        filters.fuelType === "All" || carFuelType === filters.fuelType;
    ...
    <div className="pt-48 pb-32 px-6 bg-[#050505] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-24 gap-12">
          <div>
            <span className="text-[10px] text-white/40 uppercase tracking-[0.6em] font-black mb-6 block italic">
              Elite Collection
            </span>
            <h1 className="text-6xl md:text-8xl font-display font-black uppercase italic tracking-tighter mb-6 leading-none">
              The Fleet
            </h1>
            <p
              aria-live="polite"
            ...
            >
              {isLoading
                ...
                : `Currently Curating ${cars.length} Performance Assets`}
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 w-full md:w-auto">
            <div className="relative w-full md:w-80 group">
              ...
              <Search
                className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/40 group-focus-within:text-white transition-colors"
                aria-hidden="true"
              />
              <input
                id="inventory-search"
            ...
            <div
              className="flex p-1.5 bg-white/5 rounded-full border border-white/10"
              role="group"
            ...
            <div className="sticky top-32">
              ...
              <div className="space-y-12">
                <nav aria-label="Filter by manufacturer">
                  <h3 className="text-[10px] font-black uppercase tracking-widest mb-6 block text-white/60 italic">
                    Manufacturer
                  </h3>
                ...
                <fieldset>
                  <legend className="text-[10px] font-black uppercase tracking-widest mb-6 block text-white/60 italic">
                    Body Architecture
                  </legend>
                  <div className="space-y-4">
                    {bodies.map((body) => (
                      <label
                        key={body}
                        className="flex items-center gap-4 cursor-pointer group focus-within:ring-2 focus-within:ring-[#C59B6D] focus-within:ring-offset-4 focus-within:ring-offset-black rounded-lg p-1"
                      >
                        <div className="relative w-5 h-5 flex items-center justify-center">
                          <input
                            ...
                            onChange={() =>
                              setFilters((f) => ({ ...f, bodyType: body }))
                            }
                            className="peer opacity-0 absolute inset-0 cursor-pointer"
                          />
                          <div
                            className={`w-full h-full border-2 rounded transition-all ${filters.bodyType === body ? "border-white bg-white" : "border-white/10 group-hover:border-white/30"}`}
                          />
                        </div>
                        <span
                          className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${filters.bodyType === body ? "text-white" : "text-white/50 group-hover:text-white/70"}`}
                        >
                          {body}
                        </span>
                      </label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-6">
                    <label
                      ...
                      className="text-[10px] font-black uppercase tracking-widest text-white/60 italic"
                    >
                      Budget Threshold
                    </label>
                    <span
              ...
              {isLoading ? (
                ...
                >
                  {filteredCars.map((car) => (
                    ...
                    >
                      <div
                        className={`${view === "list" ? "md:w-[40%] h-64 md:h-full" : "aspect-[16/10]"} overflow-hidden relative`}
                      >
                        <img
                          src={car.image}
                      ...
                      <div className="p-10 flex-1 flex flex-col justify-between">
                        <div className="focus-within:ring-2 focus-within:ring-[#C59B6D] focus-within:ring-offset-8 focus-within:ring-offset-black rounded-xl p-2 -m-2">
                          ...
                          <div className="flex gap-6 text-[9px] text-white/50 uppercase tracking-[0.3em] font-black mb-8">
                            <span>{car.year}</span>
                            <span
                              className="w-1 h-1 bg-white/20 rounded-full my-auto"
                              aria-hidden="true"
                            />
                        ...
                        <div className="flex justify-between items-center pt-8 border-t border-white/5">
                          <div className="flex gap-3">
                            <span className="px-4 py-1.5 bg-white/5 rounded-full text-[8px] font-black uppercase tracking-widest text-white/70">
                              {car.fuelType}
                            </span>
                            <span className="px-4 py-1.5 bg-white/5 rounded-full text-[8px] font-black uppercase tracking-widest text-white/70">
                              {car.hp} HP
                  ...
                  {filteredCars.length === 0 && (
                    <motion.div
                      ...
                      role="status"
                    >
                      <p className="text-white/40 uppercase tracking-[0.5em] text-xs font-black mb-8 italic">
